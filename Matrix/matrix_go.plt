...
plot "./Go/go_ave.txt" w lp title "逐次", for [i=1:8] sprintf("./Go/go_thread%d_ave.txt", 4**i) w lp title sprintf("並列度%d", 4**i)

set title 'Matrix Operation ExecutionSpeed by Golang \(Sequential None\)'
set output "output_Matrix_no_sequential_go.svg"
plot for [i=1:8] sprintf("./Go/go_thread%d_ave.txt", 4**i) w lp title sprintf("並列度%d", 4**i)
